...
/**
 * JiraClient to communicate with atlassian server. Mostly is send http request to get http respons by JSON query.
 * @author JituĹˇka zub
 */
public class JiraClient {
    ...
    /**
     * url of atlassian server, it is inicialized on http://applifting.atlassian.net, that is applifting server
     */
    ...
    /**
     * DateTime is date of last updated issue (the last one, that was loaded in JSON query). It is inicialized on date of first issue in applifting server.
     */
    private static DateTime dateOfLastUpdatedDoneIssue = new DateTime(2013, 8, 14, 7, 39, 31, 412, DateTimeZone.UTC);
    /**
     * Key of last updated issue. (Chech dateOfLastUpdatedDoneIssue for more information)
     */
    private static String keyOfLastUpdatedIssue;
    /**
     * status of issue, that we want to get from server
     * for done task (issues) it s status = Máme to!
     */
    ...
    /**
     * Constructor for create JiraClient, it sets username and password of user, create jiraClient, create issueQueue and save here the JSON query, that we get
     * from http request. Set cartridges (queuery.size) and clear issueQueue (it's no longer needed).
     * @param userName
     * @param password
    ...
    /**
     * This method is called in doQueueOfDoneIssues() and it remember key and date of last updated issue from entered project
    ...
    /**
     * This method create issueQueue from done issues(their keys) ordered by update date
     */
    ...
    /**
     * This method call doQueueofDoneIssue - makes new queue of new issues.
     * Add new cartridges if there is
     */
